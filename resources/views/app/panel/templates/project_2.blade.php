    ...
    <div class="editor">
        <div class="content">
            @foreach ($blocks as $block)
                {!! $block->content !!}
                <script>
                    let content{{$block->order}} = document.querySelector('.content');
                    content{{$block->order}}.innerHTML += "<div class=\"editor-toolbar\"><button><a href=\"{{route('block.edit', $block->id)}}\">{{ __('edit') }}</a></button></div>"
                </script>
            @endforeach
        </div>
    </div>
